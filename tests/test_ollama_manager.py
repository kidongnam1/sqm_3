...
from features.ai.ai_fallback_policy import AiProviderPolicy, AiFallbackSettings
...
def test_router_compare_mode_tries_paid_when_ollama_fails(monkeypatch):
    """Compare mode uses paid result when the free/local side fails."""
    router = ai_fallback.MultiProviderParserV2(openai_key="paid-key")
    called = {"paid": False}

    # compare mode ON — paid is tried even when policy marks paid_openai as disabled
    monkeypatch.setattr(
        "features.ai.ai_fallback_policy.load_ai_fallback_settings",
        lambda: AiFallbackSettings(compare_free_paid_enabled=True),
    )

    policies = [
        AiProviderPolicy("ollama", True, False, True, False),
        AiProviderPolicy("paid_openai", False, True, False, True),
    ]
    monkeypatch.setattr(
...
def test_router_skips_ollama_and_does_not_call_paid_without_policy(monkeypatch):
    ...
    # compare mode OFF — sequential policy path is tested
    monkeypatch.setattr(
        "features.ai.ai_fallback_policy.load_ai_fallback_settings",
        lambda: AiFallbackSettings(compare_free_paid_enabled=False),
    )
