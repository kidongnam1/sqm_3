...
def test_router_compare_free_and_paid_marks_conflicts(monkeypatch):
    """Compare mode returns a merged raw object and records field conflicts."""
    router = ai_fallback.MultiProviderParserV2(
        groq_key="free-key",
        openai_key="paid-key",
    )
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kw: [
            _policy("groq", True),
            _policy("paid_openai", False, is_paid=True),
        ],
    )

    def _build(self, pname):
        if pname == "groq":
            return _ReturnOk("2200034590")
        if pname == "paid_openai":
            return _ReturnOk("2200034591")
        raise RuntimeError(pname)

    monkeypatch.setattr(ai_fallback.MultiProviderParserV2, "_build_parser", _build)

    raw, provider = router.parse_invoice("dummy.pdf")

    assert provider == "compare:groq+paid_openai"
    assert raw.sap_no == "2200034590"
    assert raw.ai_compare_report["requires_review"] is True
    assert raw.ai_compare_report["conflicts"][0]["field"] == "sap_no"
