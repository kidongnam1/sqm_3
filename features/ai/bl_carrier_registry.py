...
CARRIER_TEMPLATES: dict[str, CarrierTemplate] = {
    ...
    # ── HAPAG-Lloyd ───────────────────────────────────────────────
    # BL 형식: HLCUSCL260148627 (HLC + 3자리 Origin Port Code + 숫자)
    # D/O 형식: HLCUSCL260148627 (동일)
    # 탐지: "HAPAG-LLOYD" 또는 "HAPAG LLOYD" 또는 "HLAG.COM" 또는 HLCU로 시작하는 BL
    "HAPAG": CarrierTemplate(
        carrier_id="HAPAG",
        carrier_name="Hapag-Lloyd",
        detect_keywords=[
            "HAPAG-LLOYD", "HAPAG LLOYD", "HLAG.COM", "WWW.HLAG.COM",
            "HAPAG", "HLCU",
        ],
        detect_pattern=r"HAPAG.?LLOYD|HLAG\.COM|HLCU[A-Z0-9]{6,}",
        bl_extract_pattern=r"\b(HLCU[A-Z0-9]{8,14})\b",
        bl_page_scope="page0",
        bl_format_hint="HLCUSCL260148627",
        sap_page_hint="all",
        bl_no_prompt_hint=(
            "【Hapag-Lloyd Sea Waybill / D/O 전용 규칙】\n"
            "BL No 형식: HLCU + 3자리 출발항 코드 + 숫자 (예: HLCUSCL260148627, HLCUSEL230423419)\n"
            "⚠️ 반드시 'HLCU'로 시작하는 전체 번호를 그대로 추출하세요. 숫자만 추출하면 오답입니다.\n"
            "Port of Discharge: 반드시 최종 양하항(Final Port of Discharge)을 추출하세요. "
            "환적항(Transshipment Port, e.g. Busan)은 제외하고 최종 목적항(e.g. Gwangyang)만 사용."
        ),
    ),
