# -*- coding: utf-8 -*-
"""HAPAG 직접 파싱 결과 vs API 전 필드 비교"""
import sys, requests
sys.path.insert(0, r'd:\program\SQM_inventory\SQM_v866_CLEAN')

BASE = r'D:\program\SQM_inventory\SQM 4.13 입고\hapag'
BL  = f'{BASE}\\2200034659 BL.pdf'
FA  = f'{BASE}\\2200034659 FA.pdf'
PL  = f'{BASE}\\2200034659 PL.pdf'
DO  = f'{BASE}\\HLCUSCL260148627 DO.pdf'
API = "http://127.0.0.1:8769"

from parsers.document_parser_modular.parser import DocumentParser
p = DocumentParser()
bl_r  = p.parse_bl(BL, carrier_id='HAPAG')
pl_r  = p.parse_packing_list(PL, carrier_id='HAPAG')
inv_r = p.parse_invoice(FA)
do_r  = p.parse_do(DO, carrier_id='HAPAG')
...
with open(BL,'rb') as b, open(FA,'rb') as f, open(PL,'rb') as pl_f, open(DO,'rb') as d:
    resp = requests.post(f"{API}/api/inbound/onestop-upload",
        ...
        data={'use_gemini':'false','dry_run':'true','template_id':'HAPAG_LC500'}, timeout=60)
